...
venezuela_area <- venezuela_ranked %>%
    ...
    arrange() %>%
    mutate(
    alpha_value = case_when(country_group %in% c("Colombia", "Peru") ~ 0.80,
                            .default = 0.15)
    )
...
q <- ggplot(venezuela_area, aes(x = year, y = percentage_year, fill = country_group)) +
  ...
  theme_minimal(base_family = font_choice, base_size = 14) +
    ...
    geom_richtext(data = tibble(x = 2013.07, y = 40,
                      ...
                      aes(x = x, y = y, label = label),
                      fill = "#f2e0de", label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    ...
    geom_point(data = flags,
           aes(x = x, y = y),
           size = 11.4, # Adjust size to be slightly larger than image
           color = "black",
           inherit.aes = FALSE,
