...
venezuela_line <- venezuela_ranked %>%
    ...
    summarise(total_pop_group = sum(total_pop)) %>% 
    ungroup()
...
endpoint_data <- venezuela_line %>% 
    ...
    label_y = case_when(
        ...
        country_group == "Spain" ~ total_pop_group + 80000,
        country_group == "Other" ~ total_pop_group + 0,
        country_group == "Argentina" ~ total_pop_group - 0,
        country_group == "Mexico" ~ total_pop_group - 35000,
        country_group == "Dom. Rep." ~ total_pop_group - 120000,
...
p <- ggplot(data = venezuela_line, aes(x = year, y = total_pop_group, color = country_group)) +
        ...
        annotate("rect", xmin = 2013, xmax = 2018,
                 ymin = 0, ymax = 3e6,
        ...
        labs(
            title = paste0("Under Maduro's rule, ",
                           "<span style='color: #E2A80E;'>collapsing oil prices and a deepening humanitarian </span>",
                           "<br>",
                           "<span style='color: #E2A80E;'>crisis</span> drove a ",
                           "<span style='color: #CD0000;'>massive Venezuelan exodus </span>",
                           "from 2018 onwards, easing into", "<br>",
                           "a <span style='color: #0072B2;'>slowdown after 2023</span>"),
            subtitle = paste0("Total number of migrants hosted by country, (thousands)"),
        ...
        geom_richtext(
            ...
            nudge_x = 0.65,                # optional extra push right if needed
        ...
        geom_segment(data = tibble(x = 2018, xend = 2018, y = 1290e3, yend = 2600e3),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round"
        ...
        geom_segment(data = tibble(x = 2013.2, xend = 2014.2, y = 2940e3, yend = 2940e3),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round", color = "#E2A80E"
                     ) +
        geom_segment(data = tibble(x = 2016.8, xend = 2017.8, y = 2940e3, yend = 2940e3),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round", color = "#E2A80E"
        ...
        geom_richtext(data = tibble(x = 2025.07,
                                    y = 2600e3,
                                    label = paste0("<b>", "~7.8 MILLION<br>", "</b>",
        ...
        geom_richtext(data = tibble(x = 2023.1,
                      ...
                      inherit.aes = FALSE) +
        geom_richtext(data = tibble(x = 2015.5,
                                    y = 2990e3,
                                    label = paste0("<b>CRISIS BUILD-UP</b><br>",
                                                   "Gradual deterioration of<br>",
                                                   "economic and political<br>",
                                                   "environment")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0.5, vjust = 1, size = 4,
                      color = "#F0B50F", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) + 
...
png_path <- file.path(project_path, "plots", "venezuela-refugees-maduro.png")
...
venezuela_area <- venezuela_ranked %>%
    select(-c(refugees, asylum_seekers, oip, rank)) %>% 
    filter(year >= 2013) %>%   # Maduro election
    group_by(coa_name) %>%
    # Keep top 10 countries and group rest as 'Other'
    mutate(country_group = case_when(
    ...
    ungroup() %>%
    group_by(country_group, year) %>%
    summarise(percentage_year = sum(percentage)) %>% 
    ungroup() %>% 
    arrange() %>%
    mutate(
    alpha_value = if_else(country_group %in% c("Colombia", "Peru"), 1.0, 0.15)
    )

# Get 2025 country order
order_2025 <- venezuela_area %>%
  filter(year == 2025) %>%
  arrange(percentage_year) %>%          # highest on top/bottom depending on your preference
  pull(country_group)

venezuela_area <- venezuela_area %>%
  mutate(
    country_group = factor(country_group, levels = order_2025)
  )

# Get only 2025 data and order by the desired stacking (e.g. highest on top or bottom)
final_year_data <- venezuela_area %>%
  filter(year == 2025) %>%
  arrange(desc(percentage_year)) %>%  # or your preferred order
  mutate(
    cumsum_bottom = lag(cumsum(percentage_year), default = 0),     # bottom of each area
    cumsum_top    = cumsum(percentage_year),                        # top of each area
    midpoint      = (cumsum_bottom + cumsum_top) / 2                # perfect center
  )

line_color_area <- tibble(country_group = names(line_color),
                          area_colors = line_color)

# Join midpoints back to full data (so every row knows its final-year midpoint)
venezuela_area_labeled <- venezuela_area %>%
  filter(year == 2025) %>% 
  left_join(
    select(final_year_data, country_group, midpoint),
    by = "country_group"
  ) %>%
  mutate(labels = paste0(country_group, " (",
                    label_percent(
                        scale = 1,
                        accuracy = 0.1           # rounding
                    )(percentage_year), ")"),
        label_y = case_when(
        country_group == "Argentina" ~ midpoint  + 1,
        country_group == "Mexico" ~ midpoint  + 2.7,
        country_group == "Dom. Rep." ~ midpoint  + 5,
        .default = midpoint 
    )) %>% 
  left_join(line_color_area, by = "country_group") %>% 
  mutate(labels = case_when(
        country_group == "Colombia" ~  str_replace_all(labels, "^Colombia \\(", "<b>Colombia<br>("),
        country_group == "Peru" ~  str_replace_all(labels, "^Peru", "<b>Peru"),
        .default = labels
...
venezuela_area_labeled <- venezuela_area_labeled %>% 
    mutate(x_start = year + 0.1,
           x_end = year + year_shift,
           y_start = midpoint,
           y_end = label_y)

flags <- tibble(x = c(2013.45, 2015.45, 2016.15, 2022.85, 2022.85),
                y = c(40, 71, 71, 14, 44.8),
                country = c("us", "pe", "br", "co", "pe"))
...
colombia_peru_percentage <- venezuela_ranked %>%
    filter(year >= 2013, coa_name %in% c("Colombia", "Peru")) %>% 
    group_by(year) %>% 
    summarise(sum_col_per = sum(percentage)) %>% 
    filter(year >= 2018) %>% 
    summarise(average_combined_percentage = mean(sum_col_per)) %>%
    mutate(average_combined_percentage = percent(average_combined_percentage,
                                                 scale = 1, accuracy = 1)) %>% 
    pull(average_combined_percentage)

n_total <- venezuela_data %>%
  group_by(year) %>%
  filter(year >= 2013) %>%
...
```{r}
# # Install 'ggflags' package
# install.packages("ggflags", repos = c(
#   "https://jimjam-slam.r-universe.dev",
#   "https://cloud.r-project.org"))
library(ggflags)

q <- ggplot(venezuela_area, aes(x = year, y = percentage_year, fill = country_group)) +
        geom_area(aes(alpha = alpha_value), position = "stack", color = "black", linewidth = 0.3) +
        # geom_vline(xintercept = 2018, linetype = 1, linewidth = 0.6) +
        scale_alpha_identity() +
        scale_fill_manual(values = line_color) +
        scale_x_continuous(breaks = seq(2013, 2025, 1),
                           expand = expansion(mult = c(0.015, 0))) +
        scale_y_continuous(
          labels = percent_format(scale = 1),  # show as 0–100%
          breaks = seq(0, 100, 20),
          name = "Share of total hosted (%)",
          expand = expansion(mult = c(0, 0.01))
          ) +
        coord_cartesian(clip = "off", xlim = c(2013, 2025)) +
        labs(
            title = paste0("Relative burden of Venezuelan migration: ",
                           "<span style='color: #c9a500;'>Colombia</span>",
                           " and ",
                           "<span style='color: #C0392B;'>Peru</span> took the<br>",
                           "lead from the U.S. as major host countries following 2018's mass exodus"),
            subtitle = paste0("Relative share of migrants hosted by country (%)"),
            caption = paste0("<b>Data source</b>: UNHCR Refugee Data Finder (accessed January 2026)",
                             "<br>",
                             "<b>Notes</b>: Percentages represent point-in-time migrant stock and include refugees, ",
                             "asylum-seekers, and other people in need of international<br>",
                             "protection (excluding internally displaced persons). ",
                             "The absolute migrant stock numbers are shown at the top ",
                             "of the chart for context.",
                             "<br><b>Graphic</b>: The Data Decoded / @TheDataDecoded")
        ) +
  theme_minimal(base_family = font_choice, base_size = 14) +
        theme(
            ...
            #element_line(linetype = 2, color = "grey80"),
            axis.title = element_blank(),
            ...
            plot.caption = element_markdown(hjust = 0, vjust = 0, colour = "grey50",
                                            margin = margin(t = 20), lineheight = 1.25),
            plot.margin = margin(t = 18.5, r = 165, b = 13.6, l = 18.5, unit = "pt")
        ) +
    geom_richtext(data = venezuela_area_labeled,
        aes(
          x = year,           # slight nudge right of 2025
          y = label_y,             # ← uses pre-calculated center height!
          label = labels,
          colour = area_colors
        ),
    ...
    geom_segment(data = venezuela_area_labeled ,
                     aes(x = x_start, xend = x_end, y = y_start, yend = y_end,
                         color = area_colors),
                     linewidth = 0.7,
                     lineend = "round",
    ...
    geom_segment(data = tibble(x = 2013, xend = 2013, y = 4, yend = 111.25),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round"
        ) +
    geom_segment(data = tibble(x = 2015, xend = 2015, y = 7, yend = 71),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round"
        ) +
    geom_segment(data = tibble(x = 2018, xend = 2018, y = 0, yend = 111.25),
                     aes(x = x, xend = xend, y = y, yend = yend),
                     inherit.aes = FALSE,
                     linewidth = 0.6,
                     lineend = "round"
        ) +
    geom_richtext(data = tibble(x = 2013.07, y = 40,
                                label = paste0("Upon Maduro's 2013 election,<br>",
                                               "<b>over 80%</b> of Venezuelans<br>",
                                               "abroad are in the United<br>States")),
                      aes(x = x, y = y, label = label),
                      fill = "#f0dcda", label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    geom_richtext(data = tibble(x = 2015.07, y = 71,
                                label = paste0("Conditions deteriorate<br>",
                                               "rapidly and <b>migrants<br>",
                                               "move to neighboring<br>",
                                               "countries</b>, especially<br>",
                                               "(Peru and Brazil)")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    geom_richtext(data = tibble(x = 2013.07, y = 111.25,
                                label = paste0("Early shifts in Venezuela's migrant<br>",
                                               "stock under Maduro's rule foreshadow<br>",
                                               "massive post-2018 outflows")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    geom_richtext(data = tibble(x = 2018.07, y = 111.25,
                                label = paste0("Once the mass exodus begins, ",
                                               " relative shares across countries<br>",
                                               " stabilize — <b>Colombia and Peru",
                                               " absorb ~", colombia_peru_percentage,
                                               " of the migrant<br>stock</b>",
                                               " through the end 2025")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    geom_richtext(data = tibble(x = 2018.07, y = 23,
                                label = paste0("Colombia regularizes ~2M Venezuelans<br>",
                                               "through Special (2017) and Temporary<br>",
                                               "(2021) Protection permits")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    geom_richtext(data = tibble(x = 2018.07, y = 54.2,
                                label = paste0("Peru issues ~500K temporary permits,<br>",
                                               "enabling legal stay and work (mostly<br>",
                                               "in the services/construction sectors")),
                      aes(x = x, y = y, label = label),
                      fill = NA, label.color = NA, hjust = 0, vjust = 1, size = 4,
                      color = "grey20", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE) +
    ...
    geom_point(data = flags,
           aes(x = x, y = y),
           size = 12, # Adjust size to be slightly larger than image
           color = "black",
           inherit.aes = FALSE,
           position = position_nudge(0, 4)) +
    geom_flag(data = flags, aes(x = x, y = y, country = country), 
              size = 10, inherit.aes = FALSE,
              position = position_nudge(0, 4)) +
    geom_text(data = n_total,
              aes(x = year, y = y,
                  label = total),
              inherit.aes = FALSE,
              size = 4, color = "grey30") +
    annotate("rect", xmin = 2012.8, xmax = 2025.4,
                 ymin = 113, ymax = 119, fill = NA,  color = "grey30",
             linewidth = 0.6) +
    geom_richtext(data = tibble(x = 2025.5, y = 116, 
                            label = "Absolute number of<br>migrants (thousands)"),
              aes(x = x, y = y, label = label),
              fill = NA, label.color = NA, hjust = 0, size = 4,
                      color = "grey30", show.legend = FALSE, family = font_choice,
                      inherit.aes = FALSE)
...
```{r export-visualization-area}
svg_path <- file.path(project_path, "plots", "venezuela-refugees-maduro-area.svg")
...
png_path <- file.path(project_path, "plots", "venezuela-refugees-maduro-area.png")
...
![](plots/venezuela-refugees-maduro-area.svg){width=100%}