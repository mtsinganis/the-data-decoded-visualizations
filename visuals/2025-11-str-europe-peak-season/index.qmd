...
date: "2025-11-26"
categories: [STR, Europe, occupancy, AirDNA, Tourism]
...
Across the continent, school holidays and collective holiday periods concentrate in late June to early September, with the peak in July-August.
This alignment drives demand because:
...
Northern Europe has a major seasonal advantage:
the longest and brightest days of the year occur in June-August, making these months uniquely appealing for:
...
## What do the STR industry data reveal?
...
project_path <- file.path("visuals", "2025-11-str-europe-peak-season")
...
if(!file.exists(paste0(project_path, "/data/derived/combined_countries.csv"))){
...
p <- ggplot(europe_long, aes(x = month, y = country, fill = occ_norm)) +
        ...
        theme(
          ...
          plot.caption = element_text(hjust = 0, vjust = -2.7, colour = "grey50"),
          legend.title = element_text(lineheight = 1, margin = margin(b = 20, unit = "pt")),
          legend.margin = margin(l = 0),
          plot.margin = margin(t = 5.5, r = 34, b = 5.5, l = 5.5, unit = "pt")
        )
...
ggsave(plot_path, p, width = 7, height = 8.2)
```

![](plots/thumb.svg){width=100%}
```{r multiple-lines, eval = FALSE}
europe_long <- europe_long %>%
...
p <- ggplot(europe_long, aes(x = month, y = occ_norm)) +
        ...
        theme(panel.grid.major.x = element_blank(),
              ...
              plot.caption = element_markdown(hjust = 0, vjust = 0, colour = "grey50",
                                              margin = margin(t = 35)),
              plot.margin = margin(t = 4.5, r = 21, b = 20, l = 5.5, unit = "pt")
              )
...
```{r single-lines}
europe_long_combined <- europe_long %>%
                mutate(region = case_when(
                                          country %in% c("Sweden", "Norway", "Switzerland", "Austria") ~ "Northern",
                                          country %in% c("Greece", "Cyprus", "Spain", "Italy") ~ "Southern",
                                          group == "Europe" ~ "Europe",
                                          .default = ""
                ))

europe_long_combined <- europe_long_combined %>%
                  filter(region %in% c("Northern", "Southern", "Europe")) %>%
                  group_by(region, month) %>%
                  summarise(occ_norm_mean = mean(occ_norm)) %>%
                  mutate(line_color = case_when(
                                          region == "Northern" ~ "#4c6a9c",
                                          region == "Southern" ~ "#c15065",
                                          region == "Europe" ~ "grey",
                                          .default = ""
                                               ),
                         line_type = case_when(
...
europe_long_combined <- europe_long_combined %>%
                 mutate(is_on_top = case_when(
                                        region == "Northern" ~ 3,
                                        region == "Southern" ~ 2,
                                        region == "Europe" ~ 1
                 )) %>%
                 arrange(is_on_top, region, month)

region_order <- unique(europe_long_combined$region)
...
# Define the colors and labels for the legend
legend_data <- tibble(
  label = rep(c("Northern markets", "Southern markets", "European average"), each = 2), # Repeat each label twice
    color = rep(c("#4c6a9c", "#c15065", "grey"), each = 2),
  x_start = rep(c(1, 2), times = 3), # Create two distinct dummy x-coordinates
  y_start = rep(c(1, 1), times = 3)  # Use the same dummy y-coordinate
)
...
annotation_1 <- data.frame(
  ...
  label = paste0("<b style='color: #4c6a9c;'>Northern markets</b> exhibit peak occupancy", "<br>",
                 "three times a year, aligning with the February", "<br>",
                 "ski season (<b style='font-family:\"Noto Serif\"; color: #4c6a9c;'>I</b>), the summer break (<b style='font-family:\"Noto Serif\"; color: #4c6a9c;'>II</b>),", "<br>",
                 "and December holidays (<b style='font-family:\"Noto Serif\"; color: #4c6a9c;'>III</b>)."),
  color = "grey30",
  font_size = 3.5
)
...
annotation_2 <- data.frame(
  ...
  y_pos = 0.3, 
  label = paste0("<b style='color: #c15065;'>Southern markets</b> primarily rely on summer", "<br>",
                 "tourism, enjoying a  high-occupancy period from", "<br>",
                 "June through September which peaks in August (<b style='font-family:\"Noto Serif\"; color: #c15065;'>II</b>),",
                 "<br>", "maintaining medium-strong occupancy levels during", "<br>",
                 "the shoulder seasons of Spring (<b style='font-family:\"Noto Serif\"; color: #c15065;'>I</b>) and Autumn (<b style='font-family:\"Noto Serif\"; color: #c15065;'>III</b>)."),
  color = "grey30",
  font_size = 3.5
)

annotation_markers <- tibble(
  x_coord = c(2, 7, 12, 4, 8, 10),
  # Y coordinate is set slightly below the top of the chart (y=1)
  y_coord = c(with(europe_long_combined, occ_norm_mean[region == "Northern" & month == "Feb"]) + 0.045,
              with(europe_long_combined, occ_norm_mean[region == "Northern" & month == "Jul"]) + 0.045,
              with(europe_long_combined, occ_norm_mean[region == "Northern" & month == "Dec"]) + 0.045,
              with(europe_long_combined, occ_norm_mean[region == "Southern" & month == "Apr"]) + 0.045,
              with(europe_long_combined, occ_norm_mean[region == "Southern" & month == "Aug"]) + 0.045,
              with(europe_long_combined, occ_norm_mean[region == "Southern" & month == "Oct"]) + 0.045), 
  ...
  # Text Color
  text_color = rep(c("#4c6a9c", "#c15065"), each = 3),
  
  # ggtext Aesthetics (kept constant for all points)
  hjust_val = rep(0.5, 6), # Center horizontally
  vjust_val = rep(0.55, 6) # Adjust slightly down for visual centering
)
...
p <- ggplot(europe_long_combined, aes(x = month, y = occ_norm_mean)) +
        geom_line(aes(group = region, color = line_color, linewidth = 0.8)) +
        # Layer for legend
        geom_line(
            data = legend_data, 
            # Only map 'color' to the aesthetic
        ...
        scale_linewidth_identity() +
        scale_linetype_identity() +
        coord_cartesian(xlim = c(1.5, 11.5), ylim = c(0, 1.03), clip = "off") +
        theme_minimal(base_family = font_choice, base_size = 14) +
        labs(
             x = NULL, y = NULL,
             title = paste0("Divergent seasonal peaks: occupancy patterns in <b style='color: #4c6a9c;'>Northern</b>",
                            "<br>", 
                            "and <b style='color: #c15065;'>Southern</b> European short-term rental markets"),
             subtitle = paste0("Normalized monthly occupancy (Jan–Dec 2024)"),
             caption = paste0("<b>Data source</b>: AirDNA (2025)")
             ) +
        geom_richtext(
            data = annotation_1,
        ...
        theme(panel.grid.major.x = element_blank(),
              ...
              plot.title = element_markdown(hjust = 0, face = "bold", margin = margin(b = 5),
                                          lineheight = 1, size = rel(1.6)), # left-align title
              plot.subtitle = element_markdown(hjust = 0, margin = margin(t = 3, b = 16),
                                             size = rel(1)), # left-align subtitle
              plot.caption.position = "plot",
              plot.caption = element_markdown(hjust = 0, vjust = 0, colour = "grey50",
                                              margin = margin(t = 35)),
...
pct_above <- europe_long %>% 
  mutate(region = case_when(
    country %in% c("Finland", "Norway", "Switzerland", "Austria") ~ "Northern",
    country %in% c("Greece", "Cyprus", "Spain", "Italy") ~ "Southern",
    .default = "Other"
  )) %>% 
  filter(region %in% c("Northern", "Southern")) %>% 
  group_by(region) %>% 
  reframe(
    threshold = breaks,
    pct_of_year = map_dbl(threshold, ~ mean(occ_norm >= .x, na.rm = TRUE))
  )
...
plot_note_data <- tibble(
    x_pos = -0.122,
    y_pos = -0.118, # -0.19
    note_text = paste0("<b>How to read the chart</b>: Each point on the curve shows the share of the year that a market sustains at least that",
    "<br>", "level of normalized occupancy. For example, the red circle indicates that <b style='color:#c15065;'>Southern</b> markets remain above 50% of", "<br>", "maximum occupancy for approximately 40% of the year.")
)

# pct_above <- pct_above %>%
#                 mutate(line_color = case_when(
#                                               country %in% c("Finland", "Norway", "Switzerland", "Austria") ~ "#4c6a9c",
#                                               country %in% c("Greece", "Cyprus", "Spain", "Italy") ~ "#c15065",
#                                               .default = alpha("grey90", 0.3)
#                                              ),
#                        line_width = case_when(
#                                              line_color != "grey90" ~ 0.8,
#                                              .default = 0.4
...
# # Quick validation
# pct_above %>% group_by(country) %>% distinct(line_color, line_widt) %>% print(n = 26)
...
p <- ggplot(data = pct_above, aes(x = threshold, y = pct_of_year)) +
       annotate("rect", xmin = 0.43, xmax = 0.96, ymin = 0.07, ymax = 0.59, fill = alpha("#ebebeb", 0.35), color = alpha("grey50", 1)) +
        annotate("rect", xmin = 0.017, xmax = 0.41, ymin = 0.52, ymax = 0.935, fill = alpha("#ebebeb", 0.35), color = alpha("grey50", 1)) +
        geom_line(aes(group = region, color = line_color, linewidth = line_width)) +
        guides(linetype = "none") +
        scale_linewidth_identity() +
        coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off", ratio = 1) +
        scale_x_continuous(breaks = seq(0, 1, by = 0.1)) +
        scale_y_continuous(breaks = seq(0, 1, by = 0.1), labels = label_percent(scale = 100, accuracy = 1)) +
        theme_minimal(base_family = font_choice, base_size = 14) +
        labs(
             x = NULL, y = NULL,
             title = paste0("<b style='color: #4c6a9c;'>Northern</b> European short-term rental markets maintain",
             "<br>", "higher relative occupancy over a larger share of the", "<br>", "year compared to <b style='color: #c15065;'>Southern</b> markets"),
             subtitle = paste0("Share of the year that markets maintain at least X% of their peak-season",
                               "<br>", " occupancy (normalized to each market's annual maximum)."),
             caption = paste0("<b>Data source</b>: AirDNA (2025)")
             ) +
        geom_line(
            data = legend_data, 
        ...
        geom_richtext(
          data = data.frame(x = 0.606, y = 0.57),
          aes(x = x, y = y,
              label = paste0("At high occupancy thresholds,", "<br>",
              "the <b style='color:#4c6a9c;'>Northern</b> curve stays above", "<br>",
              "the <b style='color:#c15065;'>Southern</b> curve, indicating", "<br>",
              "more months with strong demand.")),
        ...
        geom_richtext(
          data = data.frame(x = 0.029, y = 0.65),
          aes(x = x, y = y,
              label = paste0("At low occupancy", "<br>", "thresholds, the ", 
                             "<b style='color:#4c6a9c;'>Northern</b><br> curve stays below the", "<br>",
              "<b style='color:#c15065;'>Southern</b> curve, indicating", "<br>", "fewer months with weak demand.")),
          fill = NA, label.color = NA,
          label.padding = unit(c(0,0,0,0), "pt"),
          size = 4,
          hjust = 0, vjust = 1,
        ...
        geom_curve(data = tibble(x = 0.285, y = 0.64583, xend = 0.285, yend = 0.768),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "#4c6a9c",
          linewidth = 0.8,
        ...
        geom_curve(data = tibble(x = 0.67, y = 0.375, xend = 0.67, yend = 0.276),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "#4c6a9c",
          linewidth = 0.8,
        ...
        geom_curve(data = tibble(x = 0.21, y = 0.7292, xend = 0.21, yend = 0.81),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "#4c6a9c",
          linewidth = 0.8,
        ...
        geom_curve(data = tibble(x = 0.81, y = 0.2083, xend = 0.81, yend = 0.13),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "#4c6a9c",
          linewidth = 0.8,
        ...
        # x-axis arrow
        geom_curve(data = tibble(x = 0.15, y = 0.03, xend = 0.35, yend = 0.03),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "grey30",
          linewidth = 0.8,
        ...
        geom_curve(data = tibble(x = 0.03, y = 0.15, xend = 0.03, yend = 0.35),
          aes(x = x, y = y, xend = xend, yend = yend),
          arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
          color = "grey30",
          linewidth = 0.8,
        ...
        annotate(
          "richtext", label = paste0("Higher", "<br>", "normalized", "<br>", "occupancy"),
          x = 0.25, y = 0.09,
          hjust = 0.5,
          color = "grey30",
          fill = NA, label.color = NA
        ...
        annotate(
          "richtext", label = paste0("Higher", "<br>", "share of", "<br>", "the year"),
          x = 0.099, y = 0.25,
          color = "grey30",
          fill = NA, label.color = NA
        ) +
        ...
        annotate(
          "point",
          x = 0.501, y = 0.398, # coordinates of the point
          size = 3.2,
          color = "#c15065"
        ) +
        geom_richtext(data = plot_note_data,
            aes(x = x_pos, y = y_pos, label = note_text),            # size = 3.5,
            color = "grey50", 
            hjust = 0,
            vjust = 1,
            size = 4.1,
        ...
        theme(panel.grid.major.x = element_line(linetype = 2),
              ...
              plot.caption = element_markdown(hjust = 0, vjust = 0, colour = "grey50",
                                              margin = margin(t = 67)),
              legend.position = c(0.947, 0.955),
              legend.justification = c(1, 1),
              plot.margin = margin(t = 4.5, r = 21, b = 13.6, l = 5.5, unit = "pt"))

p

plot_path <- paste0(project_path, "/plots", "/share_of_year_at_occupancy.svg")

ggsave(plot_path, p, width = 8.6, height = 10.99586)
```

![](plots/share_of_year_at_occupancy.svg){width=100%}

```{r calculate-height-when-asp-1}
#| eval: false
library(grid)

square_height_for_width <- function(p, width_in = 8.6) {
  tmp <- tempfile(fileext = ".pdf")
  grDevices::pdf(tmp, width = width_in, height = width_in)
  g <- ggplotGrob(p)
  grDevices::dev.off()

  panel_layout <- g$layout[g$layout$name == "panel", ]
  panel_cols   <- panel_layout$l:panel_layout$r
  panel_rows   <- panel_layout$t:panel_layout$b

  left_extra <- convertWidth(
    sum(g$widths[1:(panel_cols[1] - 1)]), "in", valueOnly = TRUE
  )
  right_extra <- convertWidth(
    sum(g$widths[(panel_cols[length(panel_cols)] + 1):length(g$widths)]),
    "in", valueOnly = TRUE
  )
  top_extra <- convertHeight(
    sum(g$heights[1:(panel_rows[1] - 1)]), "in", valueOnly = TRUE
  )
  bottom_extra <- convertHeight(
    sum(g$heights[(panel_rows[length(panel_rows)] + 1):length(g$heights)]),
    "in", valueOnly = TRUE
  )

  panel_width_in <- width_in - left_extra - right_extra
  panel_height_in <- panel_width_in  # ratio = 1

  height_in <- top_extra + panel_height_in + bottom_extra
  height_in
}

w <- 8.6
h <- square_height_for_width(p, width_in = w)
