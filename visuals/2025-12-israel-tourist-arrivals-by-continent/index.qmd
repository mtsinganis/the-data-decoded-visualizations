...
p1 <- ggplot(dat_indexed, aes(x = date, y = arrivals_thousands, color = region)) +
          annotate("rect", xmin = as.Date("2023-10-01"), xmax = as.Date("2025-12-01"),
                   ymin = -10, ymax = 210,
          ...
          labs(
            title = paste0("Israel’s inbound tourism collapsed sharply after the outbreak",
                           "<br>",
                           "of the <span style='color: #c71e1d;'>Gaza war</span> on October 7th, 2023, and it remains far", "<br>",
                           "below pre-war levels through the end of 2025"),
...
average_last_month <- dat_indexed %>% filter(date == as.Date("2025-12-01")) %>% pull(index_sep2023) %>% mean()

p2 <- ggplot(dat_indexed, aes(x = date, y = index_sep2023, color = region)) +
          annotate("rect", xmin = as.Date("2023-10-01"), xmax = as.Date("2025-12-01"),
                   ymin = -10, ymax = 200, fill = alpha("#fa8e8e", 0.16)) +
          annotate("rect", xmin = as.Date("2025-11-15"), xmax = as.Date("2025-12-15"),
                   ymin = 31, ymax = 50, fill = NA, color = "grey20", linewidth = 0.5) +
          geom_line(linewidth = 1, na.rm = TRUE) +
          geom_vline(xintercept = base_date + 30, linetype = "dashed", color = "grey40") +
          geom_vline(xintercept = as.Date("2022-03-01"), linetype = "dashed", color = "grey40") +
          ...
          labs(
            title = paste0("Tourist arrivals to Israel fell to ~25% of pre-war levels",
            "<br>", "across all source regions in October 2023, with only <span style='color: #0072B2;'>North</span>", "<br>",
            "<span style='color: #0072B2;'>America</span> recovering to about half by the end of 2025"),
          ...
          theme(
            ...
            geom_richtext(data = tibble(x = as.Date("2023-09-01"),
                        ...
                        family = font_choice,
                        inherit.aes = FALSE) +
            geom_richtext(data = tibble(x = as.Date("2025-11-01"),
                                        y = 87.5,
                                        label = paste0("Below 50% of<br>pre-war level")),
                          aes(x = x, y = y, label = label),
                          fill = NA,
                        label.color = NA,
                        hjust = 1,         
                        ...
                        family = font_choice,
                        inherit.aes = FALSE) +
            geom_richtext(data = tibble(x = as.Date("2025-01-01"),
                                        y = 75,
                                        label = paste0("<span style='color: #0072B2;'>North America<br>recovers to 58%<br>in December 2025</span>")),
                          aes(x = x, y = y, label = label),
                          fill = NA,
                        label.color = NA,
                        hjust = 1,         
            ...
            geom_point(
              data = tibble(
                ...
                index_sep2023 = dat_indexed %>% filter(date == as.Date("2025-12-01"),
            ...
            geom_curve(data = tibble(x = as.Date("2025-11-01"), y = 87.5,
                                     xend = as.Date("2025-12-15"), yend = 52),
                aes(x = x, y = y, xend = xend, yend = yend),
              arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
              color = "grey20",
              # size = 0.4,
            ...
            geom_curve(data = tibble(x = as.Date("2025-01-01"), y = 75,
                                     xend = as.Date("2025-11-10"), yend = 60),
                aes(x = x, y = y, xend = xend, yend = yend),
              arrow = arrow(length = unit(0.35, "cm"), angle = 23, type = "closed"),
              color = "#0072B2",
              # size = 0.4,
