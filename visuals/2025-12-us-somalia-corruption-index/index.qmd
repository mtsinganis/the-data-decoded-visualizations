...
# install.packages(c("httr", "readxl")) # Uncomment and run if you don't have them
library(httr)
library(readxl)
library(dplyr)
...
extract_dir <- file.path(project_path, "data") # Name of the folder to extract contents into

# --- 1. Download the ZIP file ---

message("1. Downloading the ZIP file...")

# Use GET() from httr to handle the download safely
...
# Use unzip() from base R's 'utils' package
...
excel_filename <- list.files(extract_dir) # The file inside the zip
...
# Read the 'CPI Scores' sheet, which contains the main historical data
# The data typically starts after a few header rows (e.g., row 4 or 5)
cpi_data <- read_excel(
  path = excel_path, 
  sheet = "CPI Historical", 
  skip = 2,           # Skip the first 2 rows of headers/notes
  col_names = TRUE    # Use the third row as column names
)
